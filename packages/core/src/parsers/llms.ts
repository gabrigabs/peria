...
const FILE_PATH_PATTERN = /(?:^|\s)((?:[.\/]?[a-zA-Z0-9_\/-])+(?:\.(?:md|ts|tsx|js|jsx|json|yaml|yml|toml)))/gm
 ...
 */
export function parseLlmsContent(
  filePath: string,
  content: string
): LlmsSource {
  const variant = detectVariant(content)
...
function extractPages(content: string): LlmsPage[] {
  ...
  while ((match = PAGE_LINK_PATTERN.exec(content)) !== null) {
