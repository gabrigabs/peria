...
const SCHEMA_PATTERN = /\b([A-Z][a-zA-Z0-9]*(?:Dto|Request|Response|Input|Output|Args|Params|Config|Options|Item|Entity|Model)\b)/g
...
function extractRouteMentions(content: string): RouteMention[] {
  ...
  while ((standaloneMatch = standalonePathPattern.exec(content)) !== null) {
...
function extractSchemaReferences(content: string): SchemaReference[] {
  ...
  while ((match = SCHEMA_PATTERN.exec(content)) !== null) {
    const name = match[1]
...
function extractSourceLinks(content: string): SourceRef[] {
  ...
  while ((match = SOURCE_LINK_PATTERN.exec(content)) !== null) {
    const url = match[2]
