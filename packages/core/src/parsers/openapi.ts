...
export async function parseOpenAPI(filePath: string): Promise<OpenAPISource> {
  ...
  const securitySchemes = Object.keys((spec.components as Record<string, unknown>)?.securitySchemes || {})
 ...
 */
export async function parseOpenAPIDetailed(
  filePath: string
): Promise<{
  ...
  // Extract schemas
  const schemaComponents = components?.schemas as Record<string, Record<string, unknown>> | undefined
  if (schemaComponents) {
  ...
  if (paths) {
    for (const [path, pathMethods] of Object.entries(paths)) {
      for (const [method, operation] of Object.entries(pathMethods)) {
        ...
        const requestBody = extractRequestBody(op.requestBody as Record<string, unknown> | undefined, spec)

        // Extract responses
        const responses = extractResponses(op.responses as Record<string, unknown> | undefined, spec)

        // Extract security
        const security = extractSecurity(op.security as unknown[] | undefined, components?.securitySchemes as Record<string, unknown>)
...
function extractRequestBody(
  ...
  const jsonContent = (content?.['application/json'] || content?.['application/ld+json']) as Record<string, unknown>
 ...
 */
function resolveRef(ref: string | undefined, spec: Record<string, unknown>): Record<string, unknown> | undefined {
  ...
  const components = spec.components as Record<string, Record<string, Record<string, unknown>>> | undefined
