...
): Promise<Array<{ path: string; spec: Awaited<ReturnType<typeof parseOpenAPI>>; operations: OpenAPIOperation[] }>> {
  const results = []
...
async function scanLlms(
  ...
  return {
    ...
    content: '',
    variant: 'unknown',
    metadata: { exists: false, pageCount: 0, sectionCount: 0, hasUrls: false, filePaths: [], lineCount: 0, variant: 'unknown' },
  }
}
 ...
 */
function docToEntity(path: string, parsed: Awaited<ReturnType<typeof parseMarkdown>>): DocPageEntity {
  ...
  return {
    ...
    routeMentions: (parsed.metadata as { routeMentions?: DocPageEntity['routeMentions'] })?.routeMentions || [],
    schemaRefs: (parsed.metadata as { schemaRefs?: DocPageEntity['schemaRefs'] })?.schemaRefs || [],
    sourceRefs: (parsed.metadata as { sourceLinks?: DocPageEntity['sourceRefs'] })?.sourceLinks || [],
 ...
 * Detect framework
 */
async function detectFramework(cwd: string, config: ResolvedPeriaConfig): Promise<FrameworkMetadata | undefined> {
 ...
 */
function buildOpenAPIMetadata(results: Array<{ path: string; spec: { version?: string; metadata?: { title?: string; description?: string; paths?: string[]; schemas?: string[] } }; operations: OpenAPIOperation[] }>): OpenAPIMetadata | undefined {
 ...
 * Build docs metadata
 */
function buildDocsMetadata(results: Array<{ path: string; content: { headings?: unknown[]; metadata?: { routeMentions?: unknown[]; schemaRefs?: unknown[] } } }>): DocsMetadata {
  return {
    paths: results.map(r => r.path),
    pagesCount: results.length,
    totalHeadings: results.reduce((sum, r) => sum + (r.content.headings?.length || 0), 0),
    totalRouteMentions: results.reduce((sum, r) => sum + (r.content.metadata?.routeMentions?.length || 0), 0),
    totalSchemaRefs: results.reduce((sum, r) => sum + (r.content.metadata?.schemaRefs?.length || 0), 0),
  }
}
