...
function createPages(input: {
  ...
  return [
    createPage('overview', `${input.config.project.name} Overview`, input.config.project.description, renderOverview(input), [
      'README.md',
      'CLAUDE.md',
      'AGENTS.md',
    ]),
    createPage('packages', 'Packages', 'Workspace packages, manifests, scripts, dependencies, and package exports.', renderPackages(input), [
      'package.json',
      'packages/*/package.json',
    ]),
    createPage('cli', 'CLI Commands', 'Registered Peria CLI commands and their command registry provenance.', renderCliCommands(input), [
      'packages/cli/src/index.ts',
      'packages/cli/src/commands',
    ]),
    createPage('modules', 'TypeScript Modules', 'Source files, imports, and exported declarations extracted with ts-morph.', renderModules(input), [
      'packages/*/src/**/*.ts',
    ]),
    createPage('adapters', 'Adapters', 'Framework adapters that expose the generated docs in application runtimes.', renderAdapters(input), [
      'packages/adapters/src',
    ]),
    createPage('configuration', 'Configuration And Features', 'Resolved config, enabled features, and config provenance.', renderConfiguration(input.config, input.features), [
      'peria.config.ts',
      'packages/core/src/types/config.ts',
    ]),
    createPage('wiki-ui', 'Wiki UI', 'Static visual wiki shell generated into /docs and backed by markdown pages.', renderWikiUi(input.config), [
      'packages/docs-ui/src/index.ts',
      'packages/cli/src/commands/build.ts',
    ]),
    createPage('history', 'History', 'Recent Git history captured as dated provenance for the generated wiki.', renderHistory(input), [
      '.git',
    ]),
    createPage('ai-context', 'AI Context Map', 'A compact reading map for agents, derived from the human wiki.', renderAiContext(input), [
      ...input.contextFiles.map((file) => file.path),
      'docs/wiki-manifest.json',
    ]),
  ]
}

function createManifest(config: ResolvedPeriaConfig, generatedAt: string, git: GitMetadata, pages: WikiPage[]): WikiManifest {
  return {
...
function createGraph(input: {
  ...
  function claim(subject: string, predicate: string, object: string, provenance: Provenance, confidence: Claim['confidence'] = 'high'): Claim {
    claimCount += 1
  ...
  for (const pkg of input.packages) {
    ...
    entity(id, pkg.name, 'package', pkg.manifestPath, pkg.description ?? `Workspace package ${pkg.name}`, [
      claim(id, 'defined_in', pkg.manifestPath, provenance(pkg.manifestPath, 1, input.commit)),
      claim(id, 'has_scripts', Object.keys(pkg.scripts).join(', ') || 'none', provenance(pkg.manifestPath, 1, input.commit)),
    ])
  }
  ...
  for (const module of input.modules) {
    ...
    entity(id, module.path, 'source-file', module.path, `TypeScript module with ${module.exports.length} exported declarations.`, [
    ...
    for (const item of module.exports) {
      ...
      entity(exportId, item.name, 'export', module.path, `${item.kind} exported from ${module.path}.`, [
        claim(exportId, 'exported_from', module.path, provenance(module.path, item.line, input.commit)),
      ])
    }
  }
  ...
  for (const command of input.cliCommands) {
    ...
    entity(id, command.name, 'cli-command', command.source, command.description, [
      claim(id, 'registered_in', command.source, provenance(command.source, command.line, input.commit)),
      claim(id, 'described_as', command.description, provenance(command.source, command.line, input.commit)),
    ])
  }

  for (const feature of input.features) {
    const id = `feature:${feature.name}`
    entity(id, feature.name, 'feature', feature.source, `Feature flag ${feature.name} is ${feature.enabled ? 'enabled' : 'disabled'}.`, [
      claim(id, 'enabled', String(feature.enabled), provenance(feature.source, feature.line, input.commit)),
    ])
  }

  for (const adapter of input.adapters) {
    const id = `adapter:${adapter.name}`
    entity(id, adapter.name, 'adapter', adapter.source, `Framework adapter exported from ${adapter.source}.`, [
  ...
  for (const page of input.pages) {
    ...
    entity(id, page.title, 'wiki-page', page.path, page.description, [
      claim(id, 'generated_from', page.sourcePaths.join(', '), provenance(page.sourcePaths[0] ?? page.path, 1, input.commit), 'medium'),
    ])
  }
...
function createLlmsText(manifest: WikiManifest, contextFiles: ContextFileSummary[]): string {
  const pageLines = manifest.pages.map((page) => `- ${page.title}: docs/${page.path} - ${page.description}`)
  const contextLines = contextFiles.map((file) => `- ${file.path}${file.exists ? '' : ' (missing)'}`)
...
function renderOverview(input: {
  ...
  const implementedCommands = input.cliCommands.filter((command) => command.status === 'implemented').length
  const placeholderAdapters = input.adapters.filter((adapter) => adapter.status === 'placeholder').length
...
function renderAdapters(input: { adapters: AdapterSummary[]; config: ResolvedPeriaConfig }): string {
...
function createPage(slug: string, title: string, description: string, body: string, sourcePaths: string[]): WikiPage {
  return {
    title,
    slug,
    description,
...
function describeModuleGroup(pkg: PackageSummary, modules: ModuleSummary[]): string {
  ...
  const externalImports = unique(
    modules.flatMap((module) => module.imports.filter((item) => !item.startsWith('.') && !item.startsWith('@peria/')))
  )
...
function provenance(source: string, line: number | undefined, commit: string | undefined): Provenance {
  return {
    source,
    line,
    commit,
...
async function collectGitMetadata(cwd: string): Promise<GitMetadata> {
  const [commit, shortCommit, branch, author, authorEmail, authoredAt, subject, status, recentCommits] = await Promise.all([
